...
// MultiModalEmbeddingResponseWithHeader wraps the generated
// MultiModalEmbeddingResponse (the body of POST /embeddings/multimodal) so the
// arkruntime Client can populate and read the HTTP headers associated with
// the API call.
//
// The embedded MultiModalEmbeddingResponse's generated MarshalJSON /
// UnmarshalJSON are promoted to the wrapper, so json.Decode(wrapper) and
// json.Marshal(wrapper) behave exactly as if the value were the bare
// generated type.
type MultiModalEmbeddingResponseWithHeader struct {
	MultiModalEmbeddingResponse
	header http.Header
}

// SetHeader records the response's HTTP headers on the wrapper.
func (r *MultiModalEmbeddingResponseWithHeader) SetHeader(h http.Header) { r.header = h }

// GetHeader returns the HTTP headers recorded via SetHeader.
func (r *MultiModalEmbeddingResponseWithHeader) GetHeader() http.Header { return r.header }
