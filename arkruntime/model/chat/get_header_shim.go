...
// embeds the generated type plus a header, and exposes the interface methods.
//
// If this file is deleted or renamed without the _shim.go suffix, the build
// will break after the next `make vendor-chat`.
package chat

import "net/http"

// ChatCompletionResponseWithHeader wraps the generated ChatCompletionResponse
// (the body of POST /chat/completions, non-stream) so the arkruntime Client
// can populate and read the HTTP headers associated with the API call.
//
// The embedded ChatCompletionResponse's generated MarshalJSON / UnmarshalJSON
// are promoted to the wrapper, so json.Decode(wrapper) and json.Marshal(wrapper)
// behave exactly as if the value were the bare generated type.
type ChatCompletionResponseWithHeader struct {
	ChatCompletionResponse
	header http.Header
}

// SetHeader records the response's HTTP headers on the wrapper.
func (r *ChatCompletionResponseWithHeader) SetHeader(h http.Header) { r.header = h }

// GetHeader returns the HTTP headers recorded via SetHeader.
func (r *ChatCompletionResponseWithHeader) GetHeader() http.Header { return r.header }
